...
INPUT_ENABLE_KVM=${INPUT_ENABLE_KVM:-'true'}
INPUT_EXTRA_NIX_CONFIG=${INPUT_EXTRA_NIX_CONFIG:-""}
INPUT_GITHUB_ACCESS_TOKEN=${INPUT_GITHUB_ACCESS_TOKEN:-""}
INPUT_INSTALL_OPTIONS=${INPUT_INSTALL_OPTIONS:-""}
INPUT_NIX_PATH=${INPUT_NIX_PATH:-""}j
...
curl_retries=5
while ! curl -sS -o "$workdir/install" -v --fail -L "${INPUT_INSTALL_URL:-https://install.lix.systems/lix}"
do
  sleep 1
  ((curl_retries--))
